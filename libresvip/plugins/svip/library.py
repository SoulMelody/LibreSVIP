...
def get_xstudio_library_path() -> pathlib.Path:
    try:
        with winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Classes\svipfile\Shell\Open\Command"
        ) as key:
            open_command = winreg.QueryValueEx(key, "")[0]
            executable_path = pathlib.Path(
                shlex.split(open_command, posix=False)[0].strip('"')
            )
...
def read_xstudio_project(path: pathlib.Path):
    from System.IO import BinaryReader, FileAccess, FileMode, FileStream
