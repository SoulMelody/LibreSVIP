...
class MsczParser:
    def parse_note(self,
        ...
        if(pandas.notna(msNote.tied) and (msNote.tied == 0 or msNote.tied == -1)):
            tiesDict[msNote.midi].length = round((msNote.quarterbeats+msNote.duration_qb)*RESOLUTION)\
        ...
        result = Note(
            ...
            Length=round((msNote.quarterbeats+msNote.duration_qb)*RESOLUTION) - round(msNote.quarterbeats*RESOLUTION),
    ...
    def parse_track(self, 
            ...
            lyricsDict:Dict[int, str],
            slursList:List[int],
            tiesDict:Dict[int, Note]) -> SingingTrack:
        return SingingTrack(
    ...
    def parse_project(self, score:ms3.Score) -> Project:
        ...
        if("Chord/Spanner:type" in score.mscx.events().columns and "Chord/Spanner/Slur/eid" in score.mscx.events().columns):
            slursList = Enumerable(score.mscx.events().iterrows())\
                .select(lambda n:n[1])\
        ...
        tiesDict:Dict[int, Note] = {} #{tone, the previous note of this tone within an incomplete tie}

        trackList = Enumerable(score.mscx.notes().iterrows())\
            .select(lambda n:n[1])\
