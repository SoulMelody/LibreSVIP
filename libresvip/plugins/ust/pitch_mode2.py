...
def pitch_to_utau_mode2_track(pitch: ParamCurve, notes: list[Note], tempos: list[SongTempo]) -> UtauMode2TrackPitchData:
    ...
    dot_pit_data = [
        ...
        NotePitchData(
            to_relative(
                [point for point in absolute_pitch if notes[i].start_pos <= point.x < notes[i].end_pos],
                notes[i].key_number,
            ),
            (next((point.x for point in absolute_pitch if point.x >= notes[i].start_pos), notes[i].start_pos) - notes[i].start_pos),
    ...
    return UtauMode2TrackPitchData(notes=[
        UtauMode2NotePitchData(
            curr_note.bpm,
            milli_sec_from_tick(curr_note.offset, curr_note.bpm),
            curr_note.points[0].y / 10,
            [milli_sec_from_tick(next_point.x - prev_point.x, curr_note.bpm) for prev_point, next_point in zip(curr_note.points, curr_note.points[1:])],
            [point.y / 10 for point in curr_note.points[1:]],
            [''] * (len(curr_note.points) - 1),
            None,
        ) if curr_note.points else None
        for curr_note in dot_pit_data_simplified
    ])


def pitch_from_utau_mode2_track(pitch_data: UtauMode2TrackPitchData, notes: list[Note], tempos: list[SongTempo]) -> ParamCurve:
    ...
    for note, note_pitch in zip(notes, pitch_data.notes):
        points = []
        note_start_in_millis = tick_time_transformer.get_actual_secs_from_ticks(note.start_pos) * 1000
        if note_pitch.start is not None:
            ...
            start_shift = round(
                (last_note.key_number - note.key_number) * 100 if last_note and note.start_pos == last_note.end_pos else
                note_pitch.start_shift * 10 if note_pitch.start_shift is not None else
                0
            )
            points.append(Point(x=round(tick_pos), y=start_shift))
            for width, shift, curve_type in itertools.zip_longest(note_pitch.widths, note_pitch.shifts, note_pitch.curve_types):
                if width is None:
                    break
                if shift is None:
                    shift = 0
                ...
                if this_point.x != last_point.x and this_point.y != last_point.y:
                    interpolated_point_list = interpolate(last_point, this_point, curve_type, MODE2_PITCH_SAMPLING_INTERVAL_TICK)
                    points.extend(interpolated_point_list[1:])
                else:
                    points.append(this_point)
        pitch_points.extend(
            point for point in pending_pitch_points if point.x < (points[0].x if points else float('inf'))
        )
        pending_pitch_points = (
            shape(
    ...
    return RelativePitchCurve(points=Points(root=pitch_points)).to_absolute(notes)

def fix_points_at_last_note(pitch_data: list[Point], this_note: Note, last_note: Optional[Note]) -> list[Point]:
    if last_note is None or last_note.end_pos != this_note.start_pos:
        return pitch_data
    fixed = [Point(x=point.x, y=point.y + (this_note.key_number - last_note.key_number) * 100) if point.x < this_note.start_pos else point for point in pitch_data]
