...
class USTParser:
    ...
    mode1_track_pitch_data: UtauMode1TrackPitchData = dataclasses.field(default_factory=UtauMode1TrackPitchData)
    mode2_track_pitch_data: UtauMode2TrackPitchData = dataclasses.field(default_factory=UtauMode2TrackPitchData)

    def parse_project(self, ust_project: UTAUProject) -> Project:
        if ust_project.track:
            tempos, notes = self.parse_notes(ust_project.tempo, ust_project.track.notes)
            track = SingingTrack(
                note_list=notes,
            )
            if ust_project.pitch_mode2:
                track.edited_params.pitch = pitch_from_utau_mode2_track(
                    self.mode2_track_pitch_data,
                    track.note_list, tempos
                )
            else:
    ...
    def parse_notes(self, initial_tempo: list[str], notes: list[UTAUNote]) -> tuple[list[SongTempo], list[Note]]:
