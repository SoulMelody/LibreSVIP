...
@dataclass
class UtauNoteVibratoParams:
    length: float  # percentage of the note's length
    period: float  # milliSec
    depth: float  # cent
    fade_in: float  # percentage of the vibrato's length
    fade_out: float  # percentage of the vibrato's length
    phase_shift: float  # percentage of period
    shift: float  # percentage of depth
...
def append_utau_note_vibrato(
    ...
    note_length = tick_time_transformer.get_duration_secs_from_ticks(
    ...
    sample_interval_in_millis = tick_time_transformer.get_duration_secs_from_ticks(
        start_ticks=note_start.start_pos,
    ...
    for i in range(len(note_values)):
        x, y = note_values[i]
        x_in_millis = (tick_time_transformer.get_actual_secs_from_ticks(x) * 1000) - note_start_in_millis
        ...
        if i < len(note_values) - 1:
            ...
            while pos < ((tick_time_transformer.get_actual_secs_from_ticks(next_x) * 1000) - note_start_in_millis):
