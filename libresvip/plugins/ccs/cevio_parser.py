...
from .model import CeVIOCreativeStudioProject, CeVIOData, CeVIOUnit
...
class CeVIOParser:
    ...
    def parse_project(self, ccs_project: CeVIOCreativeStudioProject) -> Project:
        ...
        unit_nodes = [unit_node for unit_node in scene_node.units.unit
                    if unit_node.category == "SingerSong"]
        group_nodes = [group_node for group_node in scene_node.groups.group
                    if group_node.category == "SingerSong"]

        tracks: list[Track] = []
        tempos = []
        time_signatures = []
        for index, unit_node in enumerate(unit_nodes):
            group_id = unit_node.group
            group = next((group for group in group_nodes if group.group_id == group_id), None)
            track_name = group.name if group is not None else None
            track, tempo_part, time_signature_part = self.parse_track(index, unit_node, track_name)
            tracks.append(track)
        ...
        time_signatures = self.merge_time_signatures(sorted(time_signatures, key=operator.attrgetter("bar_index")))
    ...
    def merge_tempos(self, tempos: list[SongTempo]) -> list[SongTempo]:
        return [
            ...
            for key, group in itertools.groupby(
                tempos, operator.attrgetter("position")
            )
        ]
    ...
    def parse_track(self, index: int, unit_node: CeVIOUnit, track_name: Optional[str]) -> tuple[SingingTrack, list[SongTempo], list[TimeSignature]]:
        ...
        for time_node in time_nodes:
            ...
            if tick is not None and numerator is not None and denominator is not None:
                ...
                time_signatures.append(TimeSignature(bar_index=int(
        ...
        for note_node in note_nodes:
            ...
            key = pitch_step + pitch_octave * 12 if pitch_step is not None and pitch_octave is not None else None
            lyric = note_node.lyric or None
            if tick_on is not None and duration is not None and key is not None and lyric is not None:
        ...
        if unit_node.song.parameter is not None and unit_node.song.parameter.log_f0 is not None:
            ...
            cevio_track_pitch_data = CeVIOTrackPitchData(events=pitch_datas, tempos=tempos, tick_prefix=tick_prefix)

        track_name = track_name or f'Track {index + 1}'
        track = SingingTrack(title=track_name, note_list=notes)
        if cevio_track_pitch_data is not None and (pitch := pitch_from_cevio_track(cevio_track_pitch_data)) is not None:
