...
class CeVIOGenerator:
    ...
    def generate_project(self, project: Project) -> CeVIOCreativeStudioProject:
        ...
        for new_unit, new_group in self.generate_tracks(project.track_list, default_tempos, default_beats, project.song_tempo_list):
    ...
    def generate_tracks(self, tracks: list[Track], tempo: CeVIOTempo, beat: CeVIOBeat, tempo_list: list[SongTempo]) -> list[tuple[CeVIOUnit, CeVIOGroup]]:
        results = []
        for track in tracks:
    ...
    def generate_pitch(self, pitch: ParamCurve, tempo_list: list[SongTempo]) -> Optional[CeVIOParameter]:
        if (data := generate_for_cevio(pitch, tempo_list, self.first_bar_length)) is not None:
            ...
            log_f0_node.data.extend(CeVIOData(
                index=each.index,
                repeat=each.repeat,
                value=each.value,
