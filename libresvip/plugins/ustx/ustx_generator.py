    ...
    def generate_pitch(part: UVoicePart, project: USTXProject, os_pitch: list[Point], first_bar_length: int = 1920) -> None:
        ...
        for i in range(len(base_pitch)):
            ...
            if os_pitch[os_pitch_pointer].y < 0:  # 间断点
                ...
                x1 = os_pitch[os_pitch_pointer].x - first_bar_length
                x2 = os_pitch[os_pitch_pointer + 1].x - first_bar_length
                y1 = os_pitch[os_pitch_pointer].y
                y2 = os_pitch[os_pitch_pointer + 1].y
                pitd.xs.append(time)
                pitd.ys.append(round((y2 - y1) * (time - x1) / (x2 - x1) + y1 - int(base_pitch[i])))  # 线性插值
