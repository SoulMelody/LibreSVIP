...
class VocalSharpParser:
    ...
    def parse_singing_tracks(self, track_list: list[VocalSharpNoteTrack]) -> list[SingingTrack]:
        ...
        for track in track_list:
            ...
            singing_track.edited_params.pitch = self.parse_pitch(track.parameter, singing_track.note_list)
    ...
    def parse_pitch(self, parameter: VocalSharpParameter, note_list: list[Note]) -> ParamCurve:
        note_boundaries = [
        ...
        for vspx_point in parameter.points:
            if isinstance(vspx_point, PIT):
                ...
                if prev_tick is None:
                    ...
                    pitch_points.append(Point(x=cur_tick, y=-100))
                note = note_list[min(bisect.bisect_left(note_boundaries, vspx_point.time), len(note_list) - 1)]
                param_freq = midi2hz(note.key_number, a4_midi=57) + vspx_point.value * 10 / (90 - note.key_number)
                if param_freq > 0 and (pit_value := hz2midi(param_freq, a4_midi=57)) > 0:
                    pitch_points.append(
                        Point(
                            x=cur_tick,
