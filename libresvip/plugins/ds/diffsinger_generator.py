...
class DiffSingerGenerator:
    ...
    def generate(self, project: Project) -> DsItem:
        PinyinUtil.load_phoneme_table(self.options.dict_name)
        synchronizer = TimeSynchronizer(project.song_tempo_list)
        if self.options.track_index < 0:
            singing_track = next((track for track in project.track_list if isinstance(track, SingingTrack)), None)
        else:
        ...
        if self.options.export_gender:
            ...
            ds_project.gender_param_curve = GenderParamUtils.encode(os_gender_param_curve, total_duration)
