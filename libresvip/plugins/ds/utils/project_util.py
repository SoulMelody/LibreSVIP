...
def split_into_segments(project: Project, min_interval: int = 400, min_length: int = 5000) -> list[tuple[float, Project, float]]:
    ...
    for i in range(1, len(track.note_list)):
        ...
        interval = cur.start_pos - prev.start_pos - prev.length
        if interval >= min_interval and cur.start_pos - interval * 0.8 - cur_seg_start >= min_length:
            ...
            pitch_points = [Point(pos - seg_note_start_pos + prepare_space, val) for pos, val in track.edited_params.pitch.points if 1920 <= pos - 1920 <= buffer[-1].start_pos + buffer[-1].length + 50]
            gender_points = [Point(pos - seg_note_start_pos + prepare_space, val) for pos, val in track.edited_params.gender.points if 1920 <= pos - 1920 <= buffer[-1].start_pos + buffer[-1].length + 50]
            ...
            result.append(((seg_note_start_pos - prepare_space) / 1000.0, segment, trailing_space / 1000.0))
    ...
    result.append(((seg_note_start_pos - prepare_space) / 1000.0, segment, 0.5))
