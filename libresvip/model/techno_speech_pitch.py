...
class TechnoSpeechParamEvent(NamedTuple):
    idx: int | None
    repeat: int | None
    value: float


class TechnoSpeechParamEventFloat(NamedTuple):
    idx: float | None
    repeat: float | None
    value: float | None

    @classmethod
    def from_event(cls, event: TechnoSpeechParamEvent) -> TechnoSpeechParamEventFloat:
        return cls(
...
class TechnoSpeechTrackPitchData:
    events: list[TechnoSpeechParamEvent]
    tempos: list[SongTempo]
    tick_prefix: int
    vibrato_amplitude_events: list[TechnoSpeechParamEvent] = dataclasses.field(default_factory=list)
    vibrato_frequency_events: list[TechnoSpeechParamEvent] = dataclasses.field(default_factory=list)
...
def pitch_from_techno_speech_track(data: TechnoSpeechTrackPitchData) -> ParamCurve | None:
    converted_points = [Point.start_point()]
    current_value = -100

    synchronizer = TimeSynchronizer(data.tempos)
    expanded_tempos = expand(data.tempos, data.tick_prefix)
    vibrato_amplitude_interval_dict = build_param_interval_dict(
        data.vibrato_amplitude_events,
        synchronizer,
        data.tick_prefix,
        expanded_tempos,
    )
    vibrato_value_interval_dict = build_wave_interval_dict(
        data.vibrato_frequency_events,
        synchronizer,
        data.tick_prefix,
        expanded_tempos,
    )
    events_normalized = shape_events(
        normalize_to_tick(
            append_ending_points(data.events),
            data.tempos,
            data.tick_prefix,
            expanded_tempos,
        )
...
def append_ending_points(
    ...
    for event in events:
        ...
        if next_pos is not None and next_pos < pos:
            result.append(TechnoSpeechParamEvent(next_pos, None, TEMP_VALUE_AS_NULL))
        result.append(TechnoSpeechParamEvent(pos, length, event.value))
        next_pos = pos + length
...
def normalize_to_tick(
    ...
    tick_prefix: int,
    expanded_tempos: list[tuple[int, int, float]] | None = None,
) -> list[TechnoSpeechParamEventFloat]:
    tempos = expanded_tempos if expanded_tempos is not None else expand(tempo_list, tick_prefix)
    events_normalized: list[TechnoSpeechParamEventFloat] = []
    current_tempo_index = 0
    next_pos = 0.0
    next_tick_pos = 0.0
    for _event in events:
        event = TechnoSpeechParamEventFloat.from_event(_event)
        pos = event.idx if event.idx is not None else next_pos
        ...
        events_normalized.append(
            TechnoSpeechParamEventFloat(tick_pos, repeat_in_ticks, event.value)
        )
    return [
        TechnoSpeechParamEventFloat(
...
def shape_events(
    events_with_full_params: list[TechnoSpeechParamEventFloat],
) -> list[TechnoSpeechParamEventFloat]:
    result: list[TechnoSpeechParamEventFloat] = []
...
def build_param_interval_dict(
    ...
    for continuous_part in _normalized_continuous_parts(
        events, synchronizer, tick_prefix, expanded_tempos
    ):
        for prev_event, next_event in more_itertools.windowed(
            more_itertools.prepend(None, continuous_part),
            2,
        ):
...
def generate_for_techno_speech(
    pitch: ParamCurve, tempos: list[SongTempo], tick_prefix: int
) -> TechnoSpeechTrackPitchData | None:
    ...
    for i, this_point in enumerate(pitch.points.root):
        ...
        if value is not None and (next_point is None or next_point.y != -100):
            events_with_full_params.append(
                TechnoSpeechParamEventFloat(float(index), float(repeat), value)
            )
    ...
    if not events:
        return None
    return TechnoSpeechTrackPitchData(events, [], tick_prefix)
...
def denormalize_from_tick(
    ...
    tempos = (
        expanded_tempos
        if expanded_tempos is not None
        else expand(shift_tempo_list(tempos_in_ticks, tick_prefix), tick_prefix)
    )
    events_with_full_params = [
    ...
    for event_double in events_with_full_params:
        ...
        events.append(
            TechnoSpeechParamEvent(round(pos), round(max(repeat, 1)), event_double.value or 0)
        )
    return events


def restore_connection(
    events: list[TechnoSpeechParamEvent], are_events_connected_to_next: list[bool]
) -> list[TechnoSpeechParamEvent]:
    new_events = []
...
def merge_events_if_possible(
    ...
    new_events: list[TechnoSpeechParamEvent] = []
    for event in events:
        if not new_events:
            ...
            if overlapped_len > 0:
                ...
                last_event = TechnoSpeechParamEvent(
...
def _normalized_continuous_parts(
    events: list[TechnoSpeechParamEvent],
    synchronizer: TimeSynchronizer,
    tick_prefix: int,
    expanded_tempos: list[tuple[int, int, float]] | None = None,
) -> list[list[TechnoSpeechParamEventFloat]]:
    return [
        normalize_to_tick(
            append_ending_points(list(continuous_part)),
