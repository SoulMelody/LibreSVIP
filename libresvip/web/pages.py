...
def main_wrapper(header: ui.header) -> Callable[[PageArguments], None]:
    def main_page(args: PageArguments) -> None:
        ...
        if app.native.main_window is not None:
            ...
            def save_settings() -> None:
                if session_id in app.storage._users:
                    ...
                    default_settings_dict = RootModel[LibreSvipWebUserSettings](
                        settings
        ...
        def options_form(attr_prefix: str, method: str) -> None:
            ...
            conversion_plugin = get_svs_plugin_by_value(attr)
            if method == "load":
        ...
        class SelectedFormats:
            ...
            def filter_input_ext(self) -> None:
                ...
                suffixes = (
                    input_plugin.info.suffixes if input_plugin is not None else (self.input_format,)
                )
                self.files_to_convert = {
                    name: info
                    for name, info in self.files_to_convert.items()
                    if info.upload_path.suffix.removeprefix(".").lower() in suffixes
                }
                self.tasks_container.refresh()
            ...
            def _add_task(
                ...
                if settings.auto_detect_input_format:
                    ...
                    detected_plugin = get_svs_plugin_by_suffix(cur_suffix)
                    current_plugin = get_svs_plugin_by_value(self.input_format)
                    if detected_plugin is not None and detected_plugin is not current_plugin:
                        self.input_format = detected_plugin.info.suffix
                ...
                if app.native.main_window is not None and hasattr(
                    ...
                    await ui.run_javascript(
                        ...
                        if (window.showOpenFilePicker) {{
                            ...
                            let suffixes = Array.from(format_desc.matchAll(/\*(\.\w+)/g), match => match[1])
        ...
        if app.native.main_window is None:
            ...
            def recycle_state() -> None:
                if hasattr(app.state, f"{context.client.id}_selected_formats"):
        ...
        header.default_slot.children.clear()
        with header:
            with ui.row().classes("w-full"):
                ...
                def toggle_menu() -> None:
                    ...
                    toggle_menu_btn.props(
                        ...
                        if "hidden" in submenu.classes
                        else "icon=arrow_drop_up"
                    )
                ...
                with (
                    ...
                    with ui.splitter(value=25, limits=(25, 30)).classes(
                        "w-full"
                    ) as settings_splitter:
                        with (
                        ...
                        with (
                            ...
                            with ui.tab_panel(lyric_replace_rules_tab):
                                ...
                                def refresh_rules(
                                    ...
                                    return [
                                        ...
                                        for i, rule in enumerate(
                                            settings.lyric_replace_rules[preset]
                                        )
                                    ]
                                ...
                                def delete_preset() -> None:
                                    ...
                                    ui.button(icon="save", on_click=save_preset).props(
                                        "round"
                                    ).tooltip(_("Save current preset"))
        ...
        def file_format_area() -> None:
            ...
            with ui.column().classes("w-full"):
                ...
                with ui.grid().classes(
                    ...
                    ).classes("min-w-[45px] max-w-[45px] w-fit aspect-square").props(
                        "round"
